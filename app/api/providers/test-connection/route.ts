...
export async function POST(req: Request) {
  ...
  try {
    const { type, accessKeyId, secretAccessKey, endpoint, region } = await req.json();
    ...
    console.error("[test-connection] 错误:", e);
    return NextResponse.json({ error: e.message || "连接测试失败" }, { status: 500 });
  }
}
